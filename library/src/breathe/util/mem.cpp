...
// ---------------------------------------------------------------------------------------------------------------------------------
// -DOC- Enable the line below if you really want to stress-test your app's memory usage, or to help find hard-to-find bugs
// ---------------------------------------------------------------------------------------------------------------------------------
...
static	void	dumpLeakReport()
{
	...
	std::memset(timeString, 0, sizeof(timeString));
	time_t  t = time(NULL);
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		if (!reservoir)
		{
			...
			std::memset(reservoir, 0, sizeof(sAllocUnit) * 256);
		...
		std::memset(au, 0, sizeof(sAllocUnit));
		...
		{
			std::memset(au->reportedAddress, 0, au->reportedSize);
		}
